...
% parametros para o exercício 1
iter_max = 100;
a = 0.002;
TOL = 1e-4; % parada do gradiente
TOL2 = 1e-7; % busca linear
epsilon = 1e-8; %epsilon do bissecao
...
for method = 1:6
    ...
    [x_1,t] = osr (fa, gfa, Ha, x01, method, iter_max, a, TOL, TOL2, epsilon);
    fprintf('(%.1fms), xmin=[%0.4f,%0.4f], f=%0.4f\n', t*1000, x_1(1,end), x_1(2,end), fa(x_1(:,end)));

    fprintf('x0=[%2d,%2d]: ',x02(1), x02(2));
    [x_2,t] = osr (fa, gfa, Ha, x02, method, iter_max, a, TOL, TOL2, epsilon);
    ...
    saveas(gcf,(strcat('./figures/01A_m0',num2str(method),'.pdf')))
    saveas(gcf,(strcat('./figures/01A_m0',num2str(method),'.png')))
end
...
for method = 1:6
    ...
    [x_1,t] = osr (fb, gfb, Hb, xb01, method, iter_max, a, TOL, TOL2, epsilon);
    fprintf('(%.1fms), xmin=[%0.4f,%0.4f], f=%0.1f\n', t*1000, x_1(1,end), x_1(2,end), fb(x_1(:,end)));

    fprintf('x0=[%2d,%2d]: ',xb02(1), xb02(2));
    [x_2,t] = osr (fb, gfb, Hb, xb02, method, iter_max, a, TOL, TOL2, epsilon);
    ...
    saveas(gcf,(strcat('./figures/01B_m0',num2str(method),'.pdf')))
    saveas(gcf,(strcat('./figures/01B_m0',num2str(method),'.png')))
end
...
for method = 1:6
    ...
    [x_1,t] = osr (fc, gfc, Hc, xc0, method, 500, 0.005, 5e-4, TOL2, epsilon);
    ...
    saveas(gcf,(strcat('./figures/02_m0',num2str(method),'.pdf')))
    saveas(gcf,(strcat('./figures/02_m0',num2str(method),'.png')))    
end
